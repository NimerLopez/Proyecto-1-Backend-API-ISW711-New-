...
route.get('/new/:id', (req, res) => {
  const { id } = req.params;
  New.findById(id).then((data) => res.json(data)).catch((err) => res.json({ message: err }))
});
    ...
    if (newssorce && newssorce.length > 0) {
      const results = await Promise.all(
        newssorce.map(async (item) => {//se recorren los recursos de la noticias
          ...
          const result = await saveRssDataToMongo(feed.items, item.user_id, item.category_id, item._id);
          return result;
        })
      );
      console.log(results);
...
//contentSnippet
async function saveRssDataToMongo(rssData, userid, categoriid, idnewsource) {
  try {
    const results = [];
